...
describe("Tests routes /users", () => {
  ...
  beforeEach(async () => {
    ...
    const userToken = await request(app).post("/login").send(mockedLoginRequestTwo);
  ...
  it("It should not be able to create a user that already exists", async () => {
    const response = await request(app).post("/users").send(mockedUserRequestTwo);
  ...
  it("It should not be able to update user without authentication", async () => {
    const response = await request(app).patch("/users").send(mockedUpdateUserRequest);
